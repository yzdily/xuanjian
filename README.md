...
提供三种安装方式，**推荐 Docker**（最省心，跨平台）。

### 方式一：Docker（推荐）

```bash
# 1. 拉取镜像（多架构：amd64 / arm64 自动匹配）
docker pull xuanjian/xuanjian:latest

# 2. 启动服务
docker run -d --name xuanjian \
  -p 7788:7788 -p 18080:18080 -p 18081:18081 \
  -v xuanjian-data:/app/data \
  --shm-size=2gb \
  xuanjian/xuanjian:latest
...
> 首次启动后访问 Web UI，在右上角「系统设置」配置 LLM API Key 即可。

### 方式二：源码安装（macOS / Linux）
...
### 方式三：Windows 安装包（小白适用）

1. 前往 [Releases](https://github.com/xuanjian/xuanjian/releases) 下载 `XuanJian-Setup-vX.X.X.exe`
2. 双击安装（无需配置 Python 环境，内置所有依赖 + Chromium）
3. 桌面双击「XuanJian」图标启动
4. 浏览器访问 `http://localhost:7788`

> Windows 安全软件可能误报（PyInstaller 打包特性），添加白名单即可。
> 如需自行打包：`iscc scripts\installer.iss`（需安装 [Inno Setup](https://jrsoftware.org/isdl.php)）
