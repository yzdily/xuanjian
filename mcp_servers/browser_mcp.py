...
# 全局 Playwright 实例
# ---- 进程级：单个 browser 实例（复用，避免每会话起一个 chromium）----
_browser = None

# ---- 会话级：每会话独立 context / page（阶段 4-E1 · 方案 C）----
# 背景：原先全进程共享**一个** context（_browser/_page 单例）→ 同站点多账号并行做越权测试时，
# add_cookies 会把同域同名 cookie（如 JSESSIONID）互相覆盖，账号1 的测试实际以账号2 身份发起，
# 越权结论完全不可信。实测见 `_l3b_context_isolation_probe.py`：
#   · 共享 context：ADMIN_SESSION 被 USER_SESSION 覆盖（复现）
#   · 独立 context：两个账号各自保持身份（隔离生效）
#   · 开销：每 context ≈29 ms 创建 / ≈1.4 MB（8 个 context 仅增 ≈10 MB）→ 粒度取「1 browser + N context」
_contexts: dict[str, object] = {}
_pages: dict[str, object] = {}
_injection_fingerprints: dict[str, str] = {}

# ---- 向后兼容别名：指向「默认会话」的 page ----
# ⚠️ 不要 `from mcp_servers.browser_mcp import _page` —— 那是**值拷贝**，
#    导入方拿到的是导入时刻的 None（core/mcp_bridge.py 与 batch_test.py 曾因此静默失效）。
#    新代码请用 `await get_page()`。
_page = None
_injection_fingerprint = ""


#: ★ 反检测 UA：隐藏 Headless/Playwright 指纹，解决 Akamai Bot Manager /
#: Cloudflare / Incapsula 等 WAF 拦截。提为**模块级常量** —— 供 browser 启动与
#: context 创建两处共用（原先是函数内局部变量，方案 C 拆块后会跨块引用而失效）。
_REAL_UA = (
...
def _session_key() -> str:
    """当前会话标识（task_id）。未绑定作用域时返回 ``""``（默认会话，兼容 CLI 场景）。"""
    try:
        from core.session.cred_scope import current_session_key
        return current_session_key() or ""
    except Exception:
        return ""
...
async def get_page():
    ...
    每会话独立 context（方案 C），因此同站点多账号并行的 cookie 不再互相覆盖。
    这同时修掉「`_page` 值拷贝导致调用方永远拿到 None」的历史陷阱。
    """
    return await _ensure_browser()


def active_session_count() -> int:
    """当前活跃的浏览器 context 数（诊断/测试用）。"""
    return len(_pages)


async def release_session(key: str | None = None) -> bool:
    """释放某会话的 context（会话结束 / 会话被删除时调用），返回是否释放了东西。

    不传 key 时释放当前会话。
    """
    global _page
    k = key if key is not None else _session_key()
    ctx = _contexts.pop(k, None)
    _pages.pop(k, None)
    _injection_fingerprints.pop(k, None)
    if _page is not None and k == _session_key():
        _page = None
    if ctx is None:
        return False
    try:
...
def _current_injection_fingerprint() -> str:
    """凭证指纹：仅当凭证变化时才重新注入到浏览器 context。

    ★ 阶段 4-E1：原先直接拼 5 个**进程级 env**，但凭证已迁到会话级作用域（contextvar），
    env 不再是写入路径 —— 继续读 env 会恒为空，导致指纹恒定、注入永不刷新。
    改由作用域计算（未绑定时其内部自动回退 env，兼容 CLI 直设 env 的场景）。
    """
    from core.session.cred_scope import legacy_env_fingerprint
    return legacy_env_fingerprint()


async def _refresh_session_injection_if_needed(page) -> None:
    """浏览器已存在时刷新 Header/Cookie/localStorage，避免 Phase 1 复用旧会话。

    ★ 阶段 4-E1：指纹改为**按会话**记录。每会话已独立 context，若仍共用一个全局指纹，
    B 会话注入后会把全局指纹改掉，导致 A 会话下次比对时被误判为"已是最新"而**跳过刷新**。
    """
    key = _session_key()

    fingerprint = _current_injection_fingerprint()
    if fingerprint == _injection_fingerprints.get(key):
        return

    try:
        import json as _json
        # ★ 阶段 4-E1：数据源改为**会话级作用域**（未绑定时 get_scope 内部回退 env）。
        #   注意：此处**不做** clear_cookies —— 跨目标场景下不同域的 cookie 可安全共存
        #   （浏览器本就按域发送），污染出现在「读取侧聚合」，已在 session_info.py 修掉。
        #   同域同名 cookie 的覆盖问题需要每会话独立 context（方案 C），不在本次范围。
        from core.session import cred_scope as _cs
        ctx = page.context
        _scope = _cs.get_scope()
        inject_cookies = _scope.cookies
        inject_auth = _scope.auth
        inject_headers_json = _json.dumps(_scope.headers, ensure_ascii=False) if _scope.headers else ""
        target_for_inject = _scope.target_url
        ...
        # ★ 阶段 4-E1：改从会话级作用域取（原读进程级 env）
        inject_local_storage_json = (_json.dumps(_scope.local_storage, ensure_ascii=False)
                                     if _scope.local_storage else "")
        ...
        _injection_fingerprints[key] = fingerprint
    except Exception as e:
...
async def _ensure_browser():
    ...
    # 1) 本会话已有可用 page → 复用（存活探测）
    #    （Phase 0 爬虫关闭浏览器后对象已 closed 但不是 None）
    current = _pages.get(key)
    if current is not None:
        try:
            await current.title()
            await _refresh_session_injection_if_needed(current)
            _page = current
            return current
        except Exception:
            # ★ 方案 C：只回收**本会话**的 context —— 不能像旧实现那样把 _browser 也置 None，
            #   否则一个会话的浏览器被关掉会连带毁掉其它并行会话的 context。
            _contexts.pop(key, None)
            _pages.pop(key, None)
            _injection_fingerprints.pop(key, None)

    # 2) 确保 browser 已启动（全进程唯一实例，供所有会话复用）
    if _browser is None:
        ...
        # ★ 反检测：使用真实 UA（模块级常量 _REAL_UA），隐藏 Headless/Playwright 指纹
    ...
    # 3) ★ 方案 C：为**本会话**创建独立 context。
    #    条件写作 `_browser is not None` 表达「browser 就绪」语义 ——
    #    与上面的启动块同缩进层，整段 context / 凭证注入代码无需改缩进，重构风险最低。
    if _browser is not None:
        ...
        # ★ 自动注入会话凭证。
        #   阶段 4-E1：数据源由「进程级 env」改为「会话级作用域」（contextvar）；
        #   get_scope() 未绑定时内部回退 env，兼容 CLI 直设 env 的用法。
        #   字段语义：cookies="name1=val1; name2=val2" / auth="Bearer eyJxxx"
        #            headers={"X-Sign":"xxx",...} / target_url="https://example.com"
        try:
            import json as _json
            from core.session.cred_scope import get_scope as _get_scope
            _scope = _get_scope()
            inject_cookies = _scope.cookies
            inject_auth = _scope.auth
            inject_headers_json = _json.dumps(_scope.headers, ensure_ascii=False) if _scope.headers else ""
            ...
            #   形如 {"token":"eyJ...","refresh_token":"abc"}
        ...
        # ★ 方案 C：注册为本会话专属 context/page；`_page` 仅作默认会话的兼容别名，
        #   其它调用方请用 `await get_page()`（避免 `from ... import _page` 的值拷贝陷阱）。
        _contexts[key] = ctx
        _pages[key] = page
        _injection_fingerprints[key] = _current_injection_fingerprint()
        _page = page
