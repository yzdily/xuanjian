...
def _build_auth_headers() -> dict:
    ...
    """
    headers: dict = {}
    # ★ 阶段 4-E1：改从**会话级作用域**取（contextvar），未绑定时 get_scope 自动回退 env。
    #   原实现直读进程级 env → 并行会话的凭证会互相覆盖/清除。
    #   注：mcp_servers 与 core 为同进程导入（非 MCP 子进程），故 contextvar 可穿透到此处。
    from core.session import cred_scope as _cs
    _scope = _cs.get_scope()
    inject_cookies = _scope.cookies
    inject_auth = _scope.auth
    custom = dict(_scope.headers or {})
