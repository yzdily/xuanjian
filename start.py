...
def start_mitmproxy():
    ...
    try:
        ...
        time.sleep(5)  # mitmproxy 启动较慢，等 5 秒
        ...
        if mitm_process.poll() is None:
            ...
            for attempt in range(3):
                try:
                    ...
                    time.sleep(2)  # 再等 2 秒重试

            if proxy_ok:
                ok(f"mitmproxy 已启动并验证可用 (PID: {mitm_process.pid}, 端口: {PROXY_PORT})")
            else:
                warn(f"mitmproxy 进程在运行 (PID: {mitm_process.pid})，但代理端口 {PROXY_PORT} 未响应")
...
def start_mitmproxy_production():
    ...
    if is_port_in_use(PROXY_PORT):
        ...
        warn("mitmproxy 可能未正常启动，将以直连模式运行")
...
def start_web():
    ...
    try:
        import uvicorn
        uvicorn.run("web.server:app", host="0.0.0.0", port=WEB_PORT, log_level="info")
