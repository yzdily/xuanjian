...
def _list_saved_sessions() -> list[dict]:
    ...
    for task_id in task_ids:
        ...
        # ★ 阶段 2：会话面板所需的派生字段（唯一来源，前端不再自行推导）
        #   title   —— 用户别名（业务命名，如「某某公司某某系统URL测试」），存 -meta.json sidecar
        #   running —— 以 `_bg_task` 未完成判定。**不能用 active**：切换会话不会中断旧会话
        #              （见 sessions_api 的 switch），所以"后台在跑"与"当前选中"是两件事。
        #   empty   —— 无目标、无功能点、无对话历史 → 前端应折叠计数，而不是逐条刷屏。
        title = ""
        meta_file = tasks_dir / f"{task_id}-meta.json"
        if meta_file.exists():
            try:
                title = (json.loads(meta_file.read_text(encoding="utf-8")).get("title") or "").strip()
            except Exception:
                title = ""
        session = _sessions.get(task_id)
        bg = getattr(session, "_bg_task", None) if session else None
        running = bool(bg is not None and not bg.done())
        phase = str(getattr(session, "phase", "") or "") if session else ""
        _chat_has_content = chat_file.exists() and chat_file.stat().st_size > 0
        # ★ 阶段 4-E1 修复（既有 bug）：原先读 `session._current_credential`，
        #   但该字段**从未被赋值**（全仓只有读取方，写入方用的是 `_inject_cookies` /
        #   `has_credentials`）→ has_credentials 恒为 False，前端 🔑 凭证角标从未显示。
        #   改为读真实的会话凭证字段。
        has_credentials = bool(session) and bool(
        ...
        result.append({
            ...
            # 阶段 4-E1 · D3 CredScope：凭证作用域。
            # 修复后凭证承载于会话级上下文（contextvar），不再与并发的其它会话共享，
            # 故恒为 "session"（本会话专属）。空串 = 该会话无凭证。
            "cred_scope": "session" if has_credentials else "",
            "empty": (not target) and total_features == 0 and not _chat_has_content,
        })
