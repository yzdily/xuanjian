...
async def credential_injection_login(request: Request):
    ...
    # ★ F3（0918 §1.4）：解析"完整 Cookie 集"要求并喂入注入器。
    #   此前 `required_cookies` 参数在 injector/login_judge 里就绪，但**全仓零生产实参**
    #   → 登录判定退化为"出现任意 auth cookie 即成功"。
    #   解析优先级：请求体显式指定 → 租户注册表 → 按域名匹配凭据 → 空集（安全默认）。
    _required: set[str] = set()
    _body_required = body.get("required_cookies")
    if isinstance(_body_required, (list, tuple, set)) and _body_required:
        _required = {str(x).strip() for x in _body_required if str(x).strip()}
    else:
        try:
            from core.credential_store import CredentialStore
            _required = CredentialStore().required_cookies_for(
                tenant=(body.get("tenant") or "").strip(), url=target_url,
            )
        except Exception as _e:
            log.warning("解析 required_cookies 失败（降级为不启用全命中判定）: %s", _e)
    if _required:
        log.info("F3 完整 Cookie 集已启用: %s", sorted(_required))

    injector = CredentialInjector(
        headless=headless, timeout=timeout, required_cookies=_required or None,
    )
