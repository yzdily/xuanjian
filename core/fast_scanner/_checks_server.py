...
# noqa: giant  — 历史存量：服务端检测规则集中，D6 Stage 2-4 逐步消化
...
from core.realtime_protocols import extract_graphql_operations
...
# --- Host 头注入变体（§3.12.2 批次1-5） ---
HOST_VARIANTS = ["evil.com", "<target>-internal", "127.0.0.1", "localhost"]

# --- GraphQL 内省/批量探测（§3.12.3 批次2-7） ---
GRAPHQL_INTROSPECTION_QUERY = '{"query":"{__schema{types{name}}}"}'
GRAPHQL_INTROSPECTION_MARKER = '"__schema"'
GRAPHQL_BATCH_QUERY = '{"query":"query A{__typename} query B{__typename} query C{__typename}"}'

# --- 竞态条件（§3.12.3 批次2-9）：并发数固定且小，红线不真发轰炸 ---
RACE_CONCURRENCY = 5
RACE_ENDPOINT_KEYWORDS = (
    "register", "signup", "coupon", "redeem", "apply", "submit",
    "order", "pay", "transfer", "withdraw", "reset", "exchange",
)
_RACE_SUCCESS_MARKERS = ("success", "ok", "true", "created",
                         "成功", "已领取", "领取成功", "兑换成功",
                         "申请成功", "操作成功", "注册成功")
...
class _ChecksServer:
    ...
    async def _check_host_header(self, target: ScanTarget) -> list[VulnFinding]:
        """Host 头注入检测（CWE-20 / CWE-644）

        将 Host 头替换为外部域名/内网地址变体，检测：
        1. 响应体回显注入的 Host（反射）→ 可被密码重置毒化/缓存投毒
        2. Location 头指向注入的 Host
        红线：仅检测，不实际构造恶意链接。
        """
        findings = []
        parsed_host = urlparse(target.url).netloc
        if not parsed_host:
            return findings
        ...
        for variant in HOST_VARIANTS:
            host = f"{parsed_host.split(':')[0]}-internal" if variant == "<target>-internal" else variant
            headers = {**target.auth_headers, **target.headers, "Host": host}
            resp = await self._request(
                target.method, target.url, headers=headers,
                content=target.body,
                rule_tag="HostHdr", payload_tag=f"Host: {host}",
            )
            if not resp or resp.status_code >= 500:
                continue
            if _is_waf_block_page(resp) or _is_business_deny(resp.text or "") or _is_auth_wall_page(resp.text or ""):
                continue
            body = resp.text or ""
            ...
            host_l = host.lower()
            body_reflected = (
                host_l in body.lower()
                and host_l not in baseline_text.lower()
                and not _bodies_similar(baseline_text, body, threshold=0.95)
            )
            location = str(resp.headers.get("Location", "")) if resp.headers else ""
            location_reflected = host_l in location.lower()
            if body_reflected or location_reflected:
                findings.append(VulnFinding(
                    vuln_type="Host头注入",
                    severity="high" if location_reflected else "medium",
                    url=target.url,
                    method=target.method,
                    detail=(f"Host 头可被替换为 '{host}' 并被服务端反射"
                            f"（{'Location 头' if location_reflected else '响应体'}），"
                            f"可能被用于密码重置投毒/缓存投毒（CWE-20/644）"),
                    evidence=(f"Host: {host}\n"
                              f"Location: {location[:200]}\n"
                              f"Body excerpt: {body[:300]}"),
                    payload=host,
                    fix_suggestion=("校验 Host 头与白名单一致，禁止将客户端可控 Host 反射到响应体或 Location；"
                                    "生成链接时使用服务端配置的规范主机名"),
                    evidence_quality="body_confirmed" if body_reflected else "header_only",
                    rule_tag="HostHdr",
                ))
                break  # 每个 host 变体只报一次
        return findings

    async def _check_graphql_abuse(self, target: ScanTarget) -> list[VulnFinding]:
        """GraphQL 内省 + 批量滥用检测（§3.12.3 批次2-7）

        1. 识别 GraphQL 端点（URL/Content-Type/请求体特征，复用 extract_graphql_operations）
        2. 内省探测：__schema 查询是否被允许（信息泄露）
        3. 批量滥用探测：一次请求执行多个 operation（配额绕过/放大攻击）
        """
        findings = []
        baseline = await self._request(
            target.method, target.url,
        ...
        ops = extract_graphql_operations(
            ...
            headers={**target.auth_headers, **target.headers},
            body=target.body or "", response_body=baseline_text,
            status_code=baseline.status_code,
        )
        if not ops and "graphql" not in target.url.lower():
            return findings

        # 内省探测
        introsp_resp = await self._request(
            "POST", target.url,
            headers={"Content-Type": "application/json", **target.auth_headers, **target.headers},
            content=GRAPHQL_INTROSPECTION_QUERY,
            rule_tag="GraphQL", payload_tag="introspection",
        )
        if introsp_resp and introsp_resp.status_code == 200 and not _is_waf_block_page(introsp_resp):
            body = introsp_resp.text or ""
            if GRAPHQL_INTROSPECTION_MARKER in body:
                findings.append(VulnFinding(
                    vuln_type="GraphQL滥用",
                    severity="medium",
                    url=target.url,
                    method="POST",
                    detail=("GraphQL 端点开放内省（introspection），攻击者可枚举全部 schema 类型/字段/参数"
                            "（信息泄露）"),
                    evidence=body[:300],
                    payload=GRAPHQL_INTROSPECTION_QUERY,
                    fix_suggestion="生产环境关闭 GraphQL introspection；仅对白名单环境开放",
                    evidence_quality="body_confirmed",
                    rule_tag="GraphQL",
                ))

        # 批量滥用探测
        batch_finding = await self._probe_graphql_batch(target, baseline_text)
        if batch_finding:
            findings.append(batch_finding)
        return findings

    async def _probe_graphql_batch(self, target: ScanTarget,
                                   baseline_text: str) -> VulnFinding | None:
        """批量/别名滥用探测：一次请求执行多个 operation（配额绕过 / 放大攻击）。"""
        resp = await self._request(
            "POST", target.url,
            headers={"Content-Type": "application/json", **target.auth_headers, **target.headers},
            content=GRAPHQL_BATCH_QUERY,
            rule_tag="GraphQL", payload_tag="batch",
        )
        if not resp or resp.status_code != 200:
            return None
        body = resp.text or ""
        if _is_waf_block_page(resp) or _is_business_deny(body) or _is_empty_data(body):
            return None
        # 批量执行特征：响应中出现多次独立执行结果（多个 data 键 / 多次 __typename）
        if body.count('"__typename"') >= 2 or body.count('"data"') >= 2:
            return VulnFinding(
                vuln_type="GraphQL滥用",
                severity="high",
                url=target.url,
                method="POST",
                detail=("GraphQL 端点支持批量执行多个 operation（一次请求执行多条查询），"
                        "可被用于批量滥用/配额绕过/放大攻击"),
                evidence=body[:300],
                payload=GRAPHQL_BATCH_QUERY,
                fix_suggestion="限制单请求 operation 数量与复杂度（depth/aliases 配额），禁用批量执行",
                evidence_quality="body_confirmed",
                rule_tag="GraphQL",
            )
        return None

    async def _check_race_condition(self, target: ScanTarget) -> list[VulnFinding]:
        """竞态条件检测（CWE-362，§3.12.3 批次2-9）

        对幂等/资金敏感类端点并发发送相同请求，若全部成功且无限流/锁冲突迹象，
        则存在竞态风险（如优惠券重复领取、余额重复扣减）。
        红线：并发数固定且小（RACE_CONCURRENCY），仅检测不放大。
        """
        findings = []
        if not any(kw in target.url.lower() for kw in RACE_ENDPOINT_KEYWORDS):
            return findings
        ...
        async def send():
            return await self._request(
                ...
                headers={**target.auth_headers, **target.headers},
                content=target.body,
                rule_tag="RaceCond", payload_tag="race",
            )

        responses = await asyncio.gather(
            *[send() for _ in range(RACE_CONCURRENCY)], return_exceptions=True)
        responses = [r for r in responses if not isinstance(r, BaseException)]
        ok = [r for r in responses if r is not None and r.status_code < 400]
        # 全部并发成功（无 429/403/409/Retry-After）才是竞态证据
        if len(ok) < RACE_CONCURRENCY or len(ok) < 2:
            return findings
        if any(
            r is not None and (
                r.status_code in (429, 403, 409)
                or "retry-after" in {str(k).lower() for k in (r.headers or {})}
            )
            for r in responses
        ):
            return findings
        success_bodies = [r.text or "" for r in ok]
        success_marker_hits = sum(
            1 for body in success_bodies
            if any(m in body for m in _RACE_SUCCESS_MARKERS)
        )
        if success_marker_hits < 2:
            return findings
        findings.append(VulnFinding(
            vuln_type="竞态条件",
            severity="medium",
            url=target.url,
            method=target.method,
            detail=(f"端点 '{target.url}' 并发 {len(ok)} 次请求全部成功且未被限流/加锁，"
                    f"存在竞态条件风险（CWE-362），可能导致资源重复领取/扣减"),
            evidence=(f"并发 {len(ok)}/{RACE_CONCURRENCY} 次均 HTTP {ok[0].status_code}: "
                      + "; ".join(f"#{i}: {len(r.text or '')} chars" for i, r in enumerate(ok[:5]))),
            payload=target.body or "",
            fix_suggestion=("对幂等/资金敏感操作使用数据库唯一约束、分布式锁或原子更新，"
                            "防止并发重复提交"),
            evidence_quality="header_only",
            rule_tag="RaceCond",
        ))
