...
"""
# noqa: giant  — 聚合引擎：多 mixin 集成 + 调度逻辑，暂不拆分（D6 Stage 2-4 消化）
...
class FastScanner(_ChecksInjection, _ChecksServer, _ChecksAuth, _ChecksP2, _SitemapIntegration):
    """本地快速规则引擎，并发检测多种漏洞类型。
    ...
    async def scan_target(
        ...
        all_rules = enabled_rules or [
            ...
            "open_redirect", "jwt",
            # §2.8 全类别补检（§3.12.2 批次1 + §3.12.3 批次2）
    ...
    def _assign_trace_ids(self, findings: list[VulnFinding]) -> None:
        ...
        _VT_TO_TAG = {
            ...
            "垂直越权": "AuthMatrix",
            # §2.8 全类别补检（§3.12）
            "NoSQL注入": "NoSQLi", "表达式注入": "ExprInj",
            "403绕过": "403Bypass", "用户枚举": "UserEnum", "频控缺失": "UserEnum",
            "验证码答案泄露": "CaptchaLeak", "批量赋值": "MassAssign",
            "Host头注入": "HostHdr", "GraphQL滥用": "GraphQL",
            "竞态条件": "RaceCond",
            # §2.8 P2 净新增 13 类
            "XSLT注入": "XSLT", "CSV公式注入": "CSVFormula",
            "CRLF响应头注入": "CRLF", "原型链污染": "ProtoPoll",
            "HTTP/2降级漏洞": "HTTP2", "HPP参数污染": "HPP",
            "点击劫持": "ClickJack", "CSP缺失": "CSP", "CSP配置缺陷": "CSP",
            "Web缓存投毒": "CachePoison", "WebSocket明文传输": "WS",
            "WebSocket跨域": "WS", "SAML断言篡改风险": "SAML",
            "子域接管": "SubTake", "依赖混淆风险": "DepConf",
        }
        for f in findings:
            if not f.trace_id:
