...
# ★ 业务码配置接线（§4 Phase 2 item 7）：
#   此前 `core.config.build_business_deny_patterns()` / `BUSINESS_CODE_DOMAINS` 已实现
#   甚至有单测，但**生产零消费** —— `_is_business_deny` 一直读 `_constants` 的硬编码，
#   即"配置存在但没生效"（假绿）。现改为配置驱动 + 硬编码兜底。
#   默认域 profile_a 与硬编码**正则等价**（tests/unit/test_business_code_keys.py 与
#   本文件的差分检查共同保证），故默认行为零变更；换域只需
#   `XUANJIAN_BUSINESS_DOMAIN=profile_b`。
_PATTERN_CACHE: dict[str, list[str]] = {}


def active_business_deny_patterns() -> list[str]:
    """当前生效的业务拒绝正则（按业务码域缓存）。

    任何异常（导入失败/配置坏）都**回退硬编码常量**，保证检测能力不因配置问题回退。
    """
    dom = (os.getenv("XUANJIAN_BUSINESS_DOMAIN") or "").strip() or "__default__"
    cached = _PATTERN_CACHE.get(dom)
    if cached is not None:
        return cached
    try:
        from core.config import resolve_business_deny_patterns

        pats = resolve_business_deny_patterns(None if dom == "__default__" else dom)
        cached = pats or BUSINESS_DENY_PATTERNS
    except Exception:
        cached = BUSINESS_DENY_PATTERNS
    _PATTERN_CACHE[dom] = cached
    return cached
...
def _is_business_deny(text: str) -> bool:
    ...
    if not text or len(text) < 5:
        return False
    for pat in active_business_deny_patterns():
...
# ★ §2.7 WAF 厂商归因（把"有没有 WAF"升级为"是哪家 + 该用哪些绕过原语"）。
#   接线点选**真实生产判定** `_is_waf_block_page`（`_checks_auth.py` / `_checks_server.py`
#   在用），而不是 `core/fuzz/base.py:_detect_waf`（全仓仅定义、零生产调用）。
#   D7 holder 模式：模块级状态放 dataclass 实例，不用 `global`。
@dataclass
class _WafState:
    last_attribution: dict = field(default_factory=dict)


_state = _WafState()


def last_waf_attribution() -> dict:
    """最近一次 WAF 拦截的厂商归因（供报告/API 展示，无拦截时为空 dict）。"""
    return dict(_state.last_attribution)

def identify_waf_from_response(resp) -> dict:
    """从响应体识别 WAF 厂商（§2.7）。任何异常都降级为空归因，绝不影响主判定。"""
    try:
        ...
        return identify(
            ...
            headers=headers,
            body=str(getattr(resp, "text", "") or "")[:8000],
        )
    except Exception:
        return {"waf": None, "confidence": 0.0, "suggested_tamper": [], "blocked": False}


def _is_waf_block_page(resp) -> bool:
    """检测响应是否为 WAF 拦截页（403/418/429/503 + 拦截关键词）。

    命中时附带厂商归因（`identify_waf_from_response`），写入模块级状态
    `_state.last_attribution` 供报告/API 消费；归因失败**不影响**返回值。
    """
    ...
    if not body:
        return False
    if not any(kw in body for kw in WAF_BLOCK_KEYWORDS):
        return False
    # 已确认是拦截页 → 补厂商归因（§2.7）。整体替换，避免 clear+update 的中间空窗。
    try:
        _state.last_attribution = identify_waf_from_response(resp)
    except Exception:
        pass
    return True
