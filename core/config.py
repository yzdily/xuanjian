...
# ============================================================
# §4 Phase 2 item 7：业务码可配置（域A/域B 抽象，替代硬编码）
# ============================================================
# 背景：_constants.BUSINESS_DENY_PATTERNS 把 JSON 业务码字段名（code/errorCode/msg/...）
# 与拒绝码值/文案硬编码成 6 条正则。不同业务域（域A 通用风格 / 域B ret/errno 风格）
# 字段名不同，硬编码无法配置。
# 这里把"字段名 + 拒绝码值 + 拒绝文案"抽象成域 profile，build_business_deny_patterns()
# 按域 materialize 出正则列表（与 _is_business_deny 的迭代形态兼容）。
# 默认域 profile_a 精确复现当前 BUSINESS_DENY_PATTERNS，零行为变更。

BUSINESS_CODE_DOMAINS: dict[str, dict] = {
    # 域A：通用风格 code/msg/success（玄鉴默认）
    "profile_a": {
        # pattern 1：主码字段 + 长拒绝码
        "code_keys": ["code"],
        "deny_codes_long": [401, 403, 500, 40100, 40300, 40001, 40003],
        # pattern 2：备选码字段 + 短拒绝码
        "alt_code_keys": ["errorCode", "errcode", "err_code", "status_code"],
        "deny_codes_short": [401, 403, 500],
        # pattern 3：消息字段 + 拒绝文案关键词
        "message_keys": ["message", "msg", "errMsg", "errmsg", "error_msg"],
        "deny_messages": [
            "未登录", "未授权", "无权限", "请登录", "登录失效", "身份验证失败",
            "token无效", "token已过期", "权限不足", "没有权限",
            "access denied", "unauthorized", "please login", "not authenticated",
            "permission denied", "token expired", "invalid token", "authentication failed",
        ],
        # pattern 4/5：成功标志字段
        "flag_keys": ["success", "status"],
        # pattern 6：通用松散 code 字段（回退兜底，保持原行为）
        "generic_code_key": "code",
        "generic_deny_codes": [401, 403],
    },
    # 域B：ret/errno/errmsg 风格（部分自研框架 / PHP 系）
    "profile_b": {
        "code_keys": ["ret"],
        "deny_codes_long": [401, 403, 500, -1, -2],
        "alt_code_keys": ["errno", "errCode", "err_code", "retCode"],
        "deny_codes_short": [401, 403, 500],
        "message_keys": ["errmsg", "errMsg", "error_msg", "message", "msg"],
        "deny_messages": [
            "未登录", "未授权", "无权限", "请重新登录", "登录已过期", "登录过期",
            "身份验证失败", "token无效", "token已过期", "权限不足", "没有权限",
            "access denied", "unauthorized", "please login", "permission denied",
            "token expired", "invalid token", "无权访问", "鉴权失败",
        ],
        "flag_keys": ["ok", "success", "status"],
        "generic_code_key": "ret",
        "generic_deny_codes": [401, 403],
    },
}

# 默认业务码域（部署侧可改写此常量切换 profile）
DEFAULT_BUSINESS_DOMAIN = "profile_a"

# 环境变量：按部署切换业务码域（不设 → 用 DEFAULT_BUSINESS_DOMAIN）
BUSINESS_DOMAIN_ENV = "XUANJIAN_BUSINESS_DOMAIN"


def _domain_keys(p: dict, field: str) -> list[str]:
    v = p.get(field)
...
# ★ §4 Phase 2 item 7 契约符号：业务码**字段名**清单（跨 profile 去重，保序）。
# 供需要"有哪些业务码字段"的消费方使用（响应体启发式解析 / 报告标注 / UI 说明）。
# 注意：这是"字段名集合"，不是判定规则；判定规则由 active_business_deny_patterns() 产出。
BUSINESS_CODE_KEYS: list[str] = sorted({
    k
    for _p in BUSINESS_CODE_DOMAINS.values()
    for _f in ("code_keys", "alt_code_keys", "generic_code_key")
    for k in _domain_keys(_p, _f)
})


def active_business_domain() -> str:
    """当前生效的业务码域（env 覆盖 > 默认）。"""
    import os as _os

    dom = (_os.getenv(BUSINESS_DOMAIN_ENV) or "").strip()
    if dom and dom in BUSINESS_CODE_DOMAINS:
        return dom
    return DEFAULT_BUSINESS_DOMAIN


def active_business_code_keys(domain: str | None = None) -> list[str]:
    """当前生效域的业务码字段名（含消息字段与成功标志字段）。"""
    dom = domain or active_business_domain()
    p = BUSINESS_CODE_DOMAINS.get(dom) or BUSINESS_CODE_DOMAINS[DEFAULT_BUSINESS_DOMAIN]
    out: list[str] = []
    for f in ("code_keys", "alt_code_keys", "generic_code_key", "message_keys", "flag_keys"):
        out.extend(_domain_keys(p, f))
    return list(dict.fromkeys(out))


def resolve_business_deny_patterns(domain: str | None = None) -> list[str]:
    """按 env/显式域 materialize 业务拒绝正则（生产消费入口）。

    与 `build_business_deny_patterns` 的区别：本函数是**生产接线用**的入口，
    会自动解析 ``XUANJIAN_BUSINESS_DOMAIN``，且保证"未知域 → 回退默认域"。
    """
    dom = (domain or "").strip() or active_business_domain()
    return build_business_deny_patterns(dom)


def build_business_deny_patterns(domain: str | None = None) -> list[str]:
    """按业务码域 profile materialize 出业务拒绝正则列表。

    输出与 _constants.BUSINESS_DENY_PATTERNS 同形态（list[str] 正则），
    可直接喂给 _is_business_deny 风格的逐条 re.search 迭代。
    默认 profile_a 精确复现当前硬编码行为。

    抽象点：字段名 / 拒绝码值 / 拒绝文案均从 profile 读取，部署侧可注册新域。
    """
    import re as _re

    p = BUSINESS_CODE_DOMAINS.get(domain or DEFAULT_BUSINESS_DOMAIN, BUSINESS_CODE_DOMAINS[DEFAULT_BUSINESS_DOMAIN])
    patterns: list[str] = []

    # pattern 1：主码字段 + 长拒绝码
    ck = "|".join(_re.escape(k) for k in p["code_keys"])
    codes_long = "|".join(str(c) for c in p["deny_codes_long"])
    patterns.append(r'"(%s)"\s*:\s*(%s)\b' % (ck, codes_long))

    # pattern 2：备选码字段 + 短拒绝码
    ack = "|".join(_re.escape(k) for k in p["alt_code_keys"])
    codes_short = "|".join(str(c) for c in p["deny_codes_short"])
    patterns.append(r'"(%s)"\s*:\s*(%s)\b' % (ack, codes_short))

    # pattern 3：消息字段 + 拒绝文案（消息值前可有任意非引号内容）
    mk = "|".join(_re.escape(k) for k in p["message_keys"])
    msgs = "|".join(_re.escape(m) for m in p["deny_messages"])
    patterns.append(r'"(%s)"\s*:\s*"(?:[^"]*?)(?:%s)"' % (mk, msgs))

    # pattern 4/5：成功标志 false（裸值 + 字符串值）
    fk = "|".join(_re.escape(k) for k in p["flag_keys"])
    patterns.append(r'"(%s)"\s*:\s*false' % fk)
    patterns.append(r'"(%s)"\s*:\s*"false"' % fk)

    # pattern 6：通用松散码字段回退（不带引号 / 单引号变体）
    gck = _re.escape(p["generic_code_key"])
    gcodes = "|".join(str(c) for c in p["generic_deny_codes"])
    patterns.append(r'\b%s["\']?\s*[:=]\s*["\']?(%s)\b' % (gck, gcodes))
