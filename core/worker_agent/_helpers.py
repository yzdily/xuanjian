"""WorkerAgent 辅助方法 Mixin（从原 core/worker_agent.py 机械拆分）。

本模块由原 worker_agent.py 拆分而来，仅做代码搬迁，逻辑不变。
包含 _WorkerAgentHelpers Mixin 类，提供以下方法（供 WorkerAgent 继承）：
- _best_api_for_vuln
- _build_group_task_message
- _find_skill
- _split_skill_content
- _detect_vuln_evidence_in_history
- _inject_lessons_by_vuln_type
- _inject_business_understanding
"""
...
class _WorkerAgentHelpers:
    """WorkerAgent 辅助方法 Mixin。

    本 Mixin 不定义 __init__，依赖 WorkerAgent.__init__ 设置的实例属性
    （self.llm / self.sitemap / self.features / self.session_info /
     self.context / self.worker_id / self.group_name 等）。
    """
