...
# ★ P3 (2026-08-28) 同 host 死循环熔断：防止 example.com/登录页等同一 URL 空转
# 连续 BROWSE_STUCK_HOST_THRESHOLD 轮 URL host 不变 → 强制 abort
# 原机制靠 STALE_ROUNDS_LIMIT=15 兜底，但 STALE 还要配合 API 增量判断，
# 纯展示页（无 API）会一直满足"有 checklist 进度"续命，最坏撑到 200 轮撑爆上下文。
BROWSE_STUCK_HOST_THRESHOLD = 3  # 同一 host 连续多少轮触发熔断
BROWSE_STUCK_PAGE_THRESHOLD = 5  # 同一完整 URL（含 path）连续多少轮触发熔断


# ---- P3 死循环熔断异常 ----

class BrowseStuckError(RuntimeError):
    """browse_N 在同一 host/page 连续多轮无进展，强制 abort 触发。"""
    ...
    async def run(self) -> AsyncGenerator[dict, None]:
        ...
        while round_num < BROWSE_WORKER_MAX_ROUNDS and not completed:
            ...
            if _last_url and round_num > 1:
                ...
                _cur_host = _p3_up(_last_url).netloc
                _p3_host_history = getattr(self, "_p3_host_history", [])
                _p3_page_history = getattr(self, "_p3_page_history", [])
                if _p3_host_history and _p3_host_history[-1] == _cur_host:
                    _p3_host_history.append(_cur_host)
                else:
                    _p3_host_history = [_cur_host]
                if _p3_page_history and _p3_page_history[-1] == _last_url:
                    _p3_page_history.append(_last_url)
                else:
                    _p3_page_history = [_last_url]
                self._p3_host_history = _p3_host_history[-BROWSE_STUCK_HOST_THRESHOLD:]
            ...
            if response.tool_calls:
                for tc in response.tool_calls:
                    ...
                    # ★ P3 死循环熔断（2026-08-28）：从工具参数提取 url 喂给熔断器
                    _p3_url = ""
                    if isinstance(args, dict):
                        _p3_url = (
                            args.get("url")
                            or args.get("page_url")
                            or args.get("target_url")
                            or args.get("href")
                            or ""
                        )
                    if _p3_url:
                        self._p3_last_url = _p3_url
