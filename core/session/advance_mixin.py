...
class AdvancePhaseMixin:
    ...
    async def _advance_phase(self, summary: str) -> AsyncGenerator[str, None]:
        ...
        if self.phase == "explore":
            ...
            # ★ testflow 挂钩③配套（v3 §七 P4）：testflow 灰度时本调用删除——
            # 爬虫后猜测式升级与 engine census 后定档（decide_mode）二选一，不能并存。
            import os as _os
            if _os.getenv("XUANJIAN_TESTFLOW_V2", "").strip().lower() in ("census", "playbook", "full"):
                log.info("[testflow] 爬虫后升级触发器已停用（定档由 engine decide_mode 承接）")
            else:
