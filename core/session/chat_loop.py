        ...
        if self.phase == "idle":
            ...
            if _is_resume_cmd and _prev_target:
                ...
                if _had_creds or _saved_cookies or _saved_auth:
                    # ★ 阶段 4-E1：恢复到**会话级作用域**（让爬虫/子 Agent 能复用登录凭证），
                    #   不再写进程级 os.environ —— 那是并行会话互相清除凭证的根源。
                    from core.session import cred_scope as _cs
                    _cs.set_scope(_cs.CredScope(
                        cookies=_saved_cookies or "",
                        auth=_saved_auth or "",
                        headers=dict(_saved_headers or {}),
                        local_storage=_cs.current_local_storage(),
                        target_url=url or "",
                        source=_cs.SOURCE_PRESET,
                        session_key=self.task_id,     # ★ 方案 C：浏览器 context 按会话隔离
                    ))
            ...
            if has_any_auth:
                self.has_credentials = True
                # ★ 阶段 4-E1 凭证隔离：写入**会话级作用域**（contextvar），不再写进程级 os.environ。
                #   原实现先 pop 掉全部 5 个注入键再写本会话值 —— 并行会话一旦注入，
                #   就会把前一个会话的凭证**彻底清除**（实测复现见 _e1_repro_crosstalk.py L1）。
                from core.session import cred_scope as _cs
                ...
                if extra_headers:
                    ...
                    _ls_items = jwt_headers_to_local_storage(extra_headers) or {}
                    if _ls_items:
                        # 与上一轮注入的 localStorage 合并（保持既有语义）
                        _ls_items = {**_cs.current_local_storage(), **_ls_items}
                _cs.set_scope(_cs.CredScope(
                    cookies=session_cookies or "",
                    auth=auth_header or "",
                    headers=dict(extra_headers or {}),
                    local_storage=_ls_items,
                    target_url=url or "",
                    source=_cs.SOURCE_MANUAL,
                    session_key=self.task_id,     # ★ 方案 C：浏览器 context 按会话隔离
                ))
                # 清理进程级遗留注入值：env 不再是写入路径，但可能残留上一版本/上一任务的旧值，
                # 留着会被未迁移的读取点（或 env 回退）读到，造成另一形态的串扰。
                _cs.clear_env_injections()
            ...
            if crawl_result:
                ...
                # ★ testflow 挂钩①（v3 Stage 1.5）：🧩 域归属事件进终端流
                # 纯分析 0 请求；~30 秒即出归属反馈（用户承诺①）。
                # 灰度 flag：XUANJIAN_TESTFLOW_V2=census|playbook|full，未设置零行为变化。
                if os.getenv("XUANJIAN_TESTFLOW_V2", "").strip().lower() in ("census", "playbook", "full"):
                    try:
                        from core.testflow.attribution import attribute_domains, attribution_summary
                        _attr_stats = attribute_domains(atomic_features, self.sitemap)
                        yield self._event("system", attribution_summary(_attr_stats))
                        _dom_counts = _attr_stats.get("domain_counts") or {}
                        _dom_lines = [
                            f"  ─ {d} 命中 {n}" for d, n in sorted(_dom_counts.items(), key=lambda kv: -kv[1]) if n
                        ]
                        if _dom_lines:
                            yield self._event("system",
                                f"🧩 域归属明细:\n" + "\n".join(_dom_lines))
                        self._testflow_attr_stats = _attr_stats
                    except Exception as _attr_e:
                        log.debug("testflow 域归属分析失败（不阻塞爬取流程）: %s", _attr_e)
                ...
                # XSS 专项扫描
                # ★ testflow 挂钩②（v3 §七 P4 收窄）：XUANJIAN_TESTFLOW_V2 启用时
                # 全局 XSS 广扫收窄为域内测试链（upload/file 域，engine 执行），
                # 全局启动先于定档的时序问题（P4）随之消解。
                _testflow_on = os.getenv("XUANJIAN_TESTFLOW_V2", "").strip().lower() in ("census", "playbook", "full")
                if _testflow_on:
                    self._xss_scanner = None
                    self._xss_task = None
                    yield self._event("system",
                        "🛡️ XSS 检测已收窄：并入 upload/file 域内测试链（testflow 模式）")
                else:
                    try:
                ...
                # ★ testflow 挂钩③（v3 §七 P4 修复）：XUANJIAN_TESTFLOW_V2 启用时本段
                # 净删除——定档迁入 engine decide_mode（census 完成后，事件序
                # 普查完成 → 归属修正 → 定档 → 相位执行）；未设 flag 时原样保留。
                if not _testflow_on:
                ...
                if self.scan_mode == "realtime" and menu_groups and len(menu_groups) >= 1:
                    ...
                    from core.session import cred_scope as _cs_scope
                    has_cookie_inject = _cs_scope.has_credentials()
