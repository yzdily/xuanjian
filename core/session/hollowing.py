"""空心化检测 — 单源判定谓词（对应 XUANJIAN_MASTER_PLAN §3.5 A3 去重）+ F8 depth_chain 扩展。
...
F8 (0903): 新增 depth_chain 审计扩展——框架级漏洞必须有下游深挖链（depth >= 1），
否则标记为 hollowed（"发现即停"也是空心化的一种）。
"""
...
# F8 — depth_chain 字段：框架级漏洞的深度挖掘链审计
FRAMEWORK_TRIGGERS: frozenset[str] = frozenset({
    "actuator_exposure",
...
def is_finding_hollowed(
    findings: list[dict[str, Any]],
    depth_map: dict[str, int] | None = None,
) -> list[str]:
    """F8: 检查框架级漏洞是否有 depth_chain（防"发现即停"）。

    Args:
        findings: 漏洞列表，每项含 vuln_type 字段
        depth_map: {vuln_type: depth} 深度映射；None 则不做 depth 检查
    ...
    for f in findings:
        ...
        if vuln_type in FRAMEWORK_TRIGGERS:
            depth = depth_map.get(vuln_type, 0)
            if depth < 1:
                violations.append(
                    f"depth_chain 不充分: {f.get('id', '?')} "
                    f"({vuln_type}) depth={depth}，需补测下游"
                )
    return violations
