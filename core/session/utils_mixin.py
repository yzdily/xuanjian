...
class UtilsMixin:
    ...
    # ============================================================
    # F11 (0903) — LOOP 漏洞链相关 nudge 词条
    # ============================================================

    LOOP_NUDGE_TEMPLATES = {
        "actuator_exposure": (
            "⚠️ 发现 Actuator 端点暴露！不要止步于此（禁止发现即停）。\n"
            "F4 LOOP 要求继续深挖：\n"
            "1. 扫描 /actuator/env 提取密码、JDBC 连接串、secret 等敏感字段\n"
            "2. 尝试 /actuator/heapdump 下载并扫描 Shiro cipherKey、DB 密码\n"
            "3. 若发现 Shiro cipherKey → 尝试 RememberMe 反序列化 RCE\n"
            "4. 记录完整漏洞链 depth_chain"
        ),
        "shiro_remmeberme_active": (
            "⚠️ 检测到 Shiro rememberMe 活跃（Set-Cookie: deleteMe）！\n"
            "F4 LOOP 要求继续深挖：\n"
            "1. 尝试默认密钥列表（kPH+bIxk5D2deZiIxcaaaA== 等）\n"
            "2. 若密钥可获取 → 构造反序列化 payload\n"
            "3. 验证 RCE（仅审计确认可利用性，不主动执行命令）"
        ),
        "heapdump_leak": (
            "⚠️ 发现 heapdump 泄露！\n"
            "F4 LOOP 要求继续深挖：\n"
            "1. 下载 heapdump（限制 100MB）\n"
            "2. 字符串扫描 cipherKey、password、jdbc、AKIA 等\n"
            "3. 与已有发现交叉关联"
        ),
    ...
    def get_loop_nudge(self, trigger: str) -> str:
        """获取 LOOP 矩阵对应的 nudge 消息。"""
        return self.LOOP_NUDGE_TEMPLATES.get(trigger, "")

    def _enforce_lifecycle_tool(self, response) -> str | None:
        """F11: 若本 turn 无 lifecycle 工具调用 → 返回 nudge 消息。

        lifecycle 工具: finish_scan, agent_finish, spawn_agent, record_coverage
        """
        LIFECYCLE_TOOLS = {"finish_scan", "agent_finish", "spawn_agent", "record_coverage"}
        tool_calls = getattr(response, "tool_calls", [])
        if not tool_calls:
            return (
                "Your previous response ended without a lifecycle tool call.\n"
                "In non-interactive mode, plain text final answers are ignored.\n"
                "Continue immediately and call exactly one tool "
                "(finish_scan, spawn_agent, or record_coverage)."
            )
        has_lifecycle = any(
            tc.get("name", tc.get("function", {}).get("name", "")) in LIFECYCLE_TOOLS
            if isinstance(tc, dict) else False
            for tc in tool_calls
        )
        if not has_lifecycle:
            return (
                "继续执行任务。调用 lifecycle 工具（finish_scan / spawn_agent / "
                "record_coverage）来推进或结束扫描。"
            )
        return None
