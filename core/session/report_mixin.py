...
class ReportMixin:
    ...
    # ★ 中期 M1 PoC 闸：confirmed finding 必带 PoC；缺 PoC 自动降级 preliminary。
    # 新增方法，不动 v1.6 已落地的核心算法（_detect_hollowing / _compute_real_completion 等）。
    # 回滚：XUANJIAN_POC_REQUIRED=off 关闭闸（保留生成能力，不强制）。
    def _attach_poc(self, finding: dict) -> dict:
        """为单条 finding 附加 PoC；缺 PoC 的 confirmed 自动降级 preliminary。"""
        import os

        if os.environ.get("XUANJIAN_POC_REQUIRED") == "off":
            return finding  # 关闭闸

        if finding.get("verdict") != "confirmed":
            return finding  # 只对 confirmed 强制

        if finding.get("poc"):  # 已有 PoC 不重复生成
            return finding

        try:
            from core.poc_gen import gen_poc

            poc = gen_poc(finding)
        except Exception as e:  # pragma: no cover
            poc = {"format": None, "content": "", "error": str(e)[:200]}
        ...
        if poc and poc.get("content"):
            ...
            finding = dict(finding)
            finding["verdict"] = "preliminary"
            finding["poc_missing_reason"] = poc.get("error", "自动 PoC 生成失败（需人工补充）")
        return finding

    def _attach_poc_to_findings(self, findings: list) -> list:
        """批量处理：confirmed 必带 PoC。"""
        if not findings:
            return findings
        return [self._attach_poc(f) for f in findings]
