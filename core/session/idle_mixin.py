...
class IdlePhaseMixin:
    ...
    async def _run_packet_test_mode(
        ...
        # ★ 阶段 4-E1：写入**会话级作用域**（contextvar），不再写进程级 os.environ ——
        #   原实现先 pop 全部 5 键再写，会让并行会话的凭证互相清除（M3 串扰）。
    ...
    def _extract_auth_for_xss(self, crawl_result: dict | None) -> tuple[dict, dict]:
        ...
        # 阶段 4-E1：优先会话字段 → 会话级作用域 → env 回退（get_scope 内已含回退）
        from core.session import cred_scope as _cs
        credentials = getattr(self, '_inject_cookies', '') or _cs.current_cookies()
