"""core.im.dingtalk — 钉钉群机器人 connector（中期 M3）。"""
...
class DingtalkConnector(IMConnector):
    ...
    def __init__(self, webhook_url: str | None = None):
        self.webhook = webhook_url or os.environ.get("XUANJIAN_DINGTALK_WEBHOOK", "")

    def send(self, target: str, content: str, *, card: dict | None = None) -> dict:
        if not self.webhook:
            return {"status": "skipped", "error": "XUANJIAN_DINGTALK_WEBHOOK 未设置"}
    ...
    def parse_command(self, raw_msg: dict) -> dict | None:
        text = (raw_msg or {}).get("content") or (raw_msg or {}).get("text") or ""
