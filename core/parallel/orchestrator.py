...
def describe_completion(real_rate: float) -> str:
    """P2-2：按真实完成率给出收尾措辞。

    低覆盖（real_rate<10%）时**不再称「扫描完成」**，改标
    「部分完成 / 覆盖不足」，避免把"没扫完"误导成"扫完了没问题"。
    """
    if real_rate < 10.0:
        return f"部分完成 / 覆盖不足（真实完成率 {real_rate:.1f}%）"
    return f"扫描完成（真实完成率 {real_rate:.1f}%）"
