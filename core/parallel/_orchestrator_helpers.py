...
# §3.8 扫描前置授权门（SOFT 门）
#
# 背景（2026-09-19 全仓复验）：`web/api/auth_gate.py` 已有
#   `verify_authorization` / `load_scope_file`，但**没有任何生产调用方**，
#   且不存在 `enforce_pre_scan_gate` 包装 → "授权 SOFT 门未前置"。
#
# 语义（与 0918 技术规格 §5 数据流一致）：
#   - 未提供 scope 文件 → 默认**告警降级放行**（SOFT）；XJ_AUTH_GATE_STRICT=1 则阻断
#   - 提供 scope 文件   → 严格校验；越域 / 过期 → AuthScopeError **阻断**
#   - XJ_AUTH_GATE=0    → 整门关闭（回滚开关）
#
# 幂等：每会话只真正校验一次（`session._pre_scan_gate_done`），
#   run_parallel_test 与 start_browser_feature_test 共用同一门。
# ============================================================
def apply_pre_scan_gate(session: "AgentSession") -> tuple[bool, dict | None]:
    """执行扫描前置授权门。

    Returns:
        ``(blocked, event)`` —— blocked=True 时调用方必须终止本阶段；
        event 为待 yield 的 system 事件（可为 None）。
    """
    if getattr(session, "_pre_scan_gate_done", False):
        return False, None
    session._pre_scan_gate_done = True
    ...
    for attr in ("target_url", "target", "url"):
        ...
        if v:
            target = str(v)
            break
    scope_file = getattr(session, "scope_file", None) or os.getenv("XUANJIAN_SCOPE_FILE") or None

    try:
        from web.api.auth_gate import enforce_pre_scan_gate, AuthScopeError
    except Exception as e:  # 导入失败不应阻断扫描（降级留痕）
        log.warning("授权门模块不可用，跳过前置授权校验: %s", e)
        return False, session._event("system", f"⚠️ 授权门不可用，已跳过（{e}）")

    try:
        info = enforce_pre_scan_gate(target, scope_file)
    except AuthScopeError as e:
        # 授权门失败必须 fail-closed：不授权就不扫
        log.error("前置授权门阻断: %s", e)
        try:
            session.sitemap.auth_gate_blocked = True
            session.sitemap.auth_gate_reason = str(e)
        except Exception:
            pass
        return True, session._event(
            "system", f"⛔ 前置授权门阻断：{e}（扫描未开始）"
        )
    except Exception as e:
        log.error("前置授权门异常，按阻断处理（fail-closed）: %s", e)
        return True, session._event("system", f"⛔ 前置授权门异常，已阻断：{e}")

    try:
        session.sitemap.auth_gate_result = info
    except Exception:
        pass
    ...
    if info.get("degraded"):
        return False, session._event("system", f"⚠️ 授权门（SOFT）：{info.get('reason')}")
    log.info("前置授权门通过: %s", info.get("reason"))
    return False, session._event("system", f"✅ 授权门：{info.get('reason')}")
