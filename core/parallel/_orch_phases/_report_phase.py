...
async def _enter_report_phase(session: "AgentSession") -> AsyncGenerator[str, None]:
    ...
    # ★ F13 (0918 §2.3)：已确认 finding 触发下游漏洞链补测（假设链 spawner）
    # 闭环点：_supplement.py 已定义 FINDING_SPAWN_PATTERNS / should_spawn_for_finding /
    # _spawn_hypothesis_agent，core/loops/hypothesis_spawner.run_spawner 也已就绪，
    # 但编排此前从不调用 → 漏洞链下游补测空转（actuator→env 等漏挖）。
    # 此处收尾钩入：收集已确认漏洞 → run_spawner → 产出下游验证任务描述，
    # 落 session 并作为报告 follow-up 暴露（防空转、不阻断报告阶段）。
    try:
        from core.loops.coverage_integration import _confirmed_vulns_from_features
        from core.loops.hypothesis_spawner import run_spawner

        _sitemap = getattr(session, "sitemap", None)
        _feats = list(_sitemap.features.values()) if _sitemap else []
        confirmed = _confirmed_vulns_from_features(_feats) if _feats else []
        _loop_covered: list = []
        if confirmed:
            # ★ 0919：F13 与 E3-1 分工 —— 矩阵已覆盖的漏洞会由确定性深挖链（Phase 2.7，
            #   就在本钩子之后执行）接管，spawner 只为"矩阵未覆盖"的漏洞出建议，
            #   避免同一漏洞被两条路径重复处理。判定逻辑单源在 core.loops.deep_dive。
            try:
                from core.loops.deep_dive import filter_covered_by_loops
                _loop_covered, confirmed = filter_covered_by_loops(confirmed)
            except Exception as _e:
                log.warning("F13/E3-1 去重判定失败（按不去重处理）: %s", _e)
            if _loop_covered:
                _types = "、".join(sorted({str(v.get("vuln_type") or "") for v in _loop_covered}))
                yield session._event(
                    "system",
                    f"🔗 F13 跳过 {len(_loop_covered)} 项（{_types}）—— 将由 Phase 2.7 确定性深挖链处理",
                )
        if confirmed:
            descs = await run_spawner(session, confirmed)
            if descs:
                session._f13_followup_tasks = list(descs)
                yield session._event(
                    "system",
                    f"🔗 F13 假设链下游补测建议 {len(descs)} 项（防空转上限已应用）",
                )
                for _d in descs:
                    yield session._event("system", f"   → {str(_d)[:160]}")
            else:
                yield session._event(
                    "system",
                    "🔗 F13 假设链：已确认漏洞均不命中 spawn 模式，无下游补测",
                )
        elif _loop_covered:
            yield session._event(
                "system",
                "🔗 F13 假设链：已确认漏洞已全部由确定性深挖链覆盖，无需额外补测建议",
            )
    except Exception as e:  # F13 钩子失败绝不影响报告
        log.warning("F13 spawner hook error: %s", e, exc_info=True)
    ...
    # ★ E3-1（0919）：Phase 2.7 · LOOP 深挖 — 对已确认漏洞命中 trigger 的执行 depth_chain。
    #   放在此处的原因：所有 VULNERABLE/CONFIRMED 都已汇总在 sitemap，且紧随其后的
    #   upsert_vuln 循环与报告渲染会自动带上深挖产出（无需改动报告链路）。
    #   开关：XJ_LOOP_ENABLED=0 关闭；XJ_LOOP_TRIGGERS 默认只开 actuator_exposure。
    if session.sitemap:
        try:
            from core.loops.deep_dive import run_deep_dive
            async for evt in run_deep_dive(session):
                yield evt
        except Exception as _e:
    ...
    # ★ G4/G5/G6/G7：覆盖骨架 + SARIF + STRIDE 确定性产物导出（容错，不影响主报告）
    cov_summary_text = ""
    if session.sitemap:
        try:
            from core.loops.coverage_integration import export_scan_artifacts
            cov_summary_text = export_scan_artifacts(session) or ""
            if cov_summary_text:
                yield session._event("system", f"📊 覆盖骨架产物已生成：{cov_summary_text}")
        except Exception as _e:
