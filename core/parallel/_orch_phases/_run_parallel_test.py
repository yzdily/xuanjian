...
#
# noqa: giant —— 本文件改造前 791 行（已贴 D6 行数门 800）；2026-09-19 因
# §1.5 Track A 按域派活 + §3.8 授权 SOFT 门两处**必要接线**增至 ~808 行。
# 适配逻辑已外移到 core/endpoint/track_a.py，编排侧仅留调用；
# 后续应把本文件的「接力 Agent 重试」段（约 90 行，独立关注点）拆为
# _retry_phase.py 以回到 800 行以内。当前按仓库既有惯例（同目录 batch_test.py
# 等 24 个文件）使用 noqa 豁免，避免为凑行数删减文档注释。
...
async def run_parallel_test(session: "AgentSession") -> AsyncGenerator[str, None]:
    ...
    # ---- Step 0: §3.8 扫描前置授权门（SOFT 门）----
    # 修复"verify_authorization 已实现但生产零调用 → 授权 SOFT 门未前置"。
    # 越域/过期 → 阻断本阶段；未提供 scope 文件 → 告警降级放行（SOFT）。
    _ag_blocked, _ag_event = apply_pre_scan_gate(session)
    if _ag_event:
        yield _ag_event
    if _ag_blocked:
        return
    ...
    if _is_packet_mode:
        # ★ 阶段 4-E1：改从**会话级作用域**取（原直接读进程级 os.environ，并行会话互相覆盖）
        from core.session import cred_scope as _cs
        _scope = _cs.get_scope()
        _headers: dict = {}
        if _scope.cookies:
            _headers["Cookie"] = _scope.cookies
        if _scope.auth:
            _headers["Authorization"] = _scope.auth
        if _scope.headers:
            _headers.update(_scope.headers)
        session_info = {"headers": _headers} if _headers else {}
    else:
        yield session._event("system", "正在获取 Session 信息...")
        # ★ L2 修复：传本会话目标 URL，只取该域下的 cookie
        from core.session import cred_scope as _cs
        session_info = await get_session_info(_cs.current_target_url())
    ...
    # ---- Step 8.5: Track A 按风险域分批派活（§1.5 P0）----
    # 修复"group_by_risk_domain 已实现但编排零调用 → 按域派活空转"；XJ_TRACK_A=0 回滚。
    feature_groups, _ta_evt = apply_track_a(session, feature_groups)
    if _ta_evt:
        yield _ta_evt
    ...
    if worker_features:
        ...
        try:
            ...
            #   → `_probe_cred` 恒为 None → 整个 F2 认证探活块被静默跳过。
            #   改为从会话级作用域构造（verify_auth_validity 期望 {headers, cookies} 结构）。
            #   注：本处仍未传 request_fn，而 verify_auth_validity 在 request_fn 为空时
            #   直接 return True（空操作）—— 属另一处独立待修项，本次不扩张范围。
            from core.session import cred_scope as _cs_probe
            _scope_probe = _cs_probe.get_scope()
            _probe_cred = None
            if _scope_probe.cookies or _scope_probe.auth:
                _probe_cred = {
                    "headers": {"Authorization": _scope_probe.auth} if _scope_probe.auth else {},
                    "cookies": _scope_probe.cookies,
                }
            if _probe_api and _probe_cred:
                verify_auth_validity(_probe_api, _probe_cred)
        except AuthInvalidError as _e:
            yield session._event("system", f"⚠️ F2 认证探活失败: {_e}")
            log.error("F2 auth probe failed: %s", _e)
        except Exception:
            pass
        ...
        while queue and len(active_workers) < _effective_max_workers:
            ...
            worker = WorkerAgent(
                worker_id=worker_id_for(group_name, worker_idx),
                llm=session.llm,
                features=group_fps,
    ...
    for retry_round in range(MAX_RETRY_ROUNDS):
        ...
        while retry_queue and len(retry_active) < _effective_max_workers:
            ...
            for fp in gfps:
                session.sitemap.start_test(fp.id)
            w = WorkerAgent(
                worker_id=worker_id_for(gname, worker_idx),
                llm=session.llm,
                features=gfps,
                group_name=gname,
                sitemap=session.sitemap,
