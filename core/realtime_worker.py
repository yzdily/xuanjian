...
class FlowWatcher:
    ...
    async def _get_session_info(self) -> dict:
        ...
        try:
            ...
            # 阶段 4-E1 · L2：传目标 URL，避免把其它会话/目标的 cookie 混进同一 Cookie 头
            return await get_session_info(_cs.current_target_url())
        except Exception:
            # 降级：走**会话级作用域**（未绑定时自动回退 env），不再直接读进程级 env
            headers = {}
            cookies = _cs.current_cookies()
            auth = _cs.current_auth()
            if cookies:
