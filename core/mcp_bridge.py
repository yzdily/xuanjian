...
from mcp_servers.browser_mcp import (
    _ensure_browser,
    browser_goto,
    get_page,
    release_session,
...
__all__ = [
    ...
    "browser_goto",
    # ★ 阶段 4-E1：不再导出 `_page`。
    #   `from mcp_servers.browser_mcp import _page` 是**值拷贝** —— 导入时刻 `_page` 是 None，
    #   导入方永远拿到 None（batch_test.py 里「从浏览器补拿 token」的逻辑曾因此静默失效）。
    #   改为函数式访问：`page = await get_page()`（返回当前会话的 page）。
    "get_page",
    "release_session",
    "_current_task_id",
    "_load_new_flows",
