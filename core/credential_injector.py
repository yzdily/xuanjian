...
class CredentialInjector:
    ...
    def __init__(
        ...
        headless: bool = False,
        timeout: int = 180,
        required_cookies: Iterable[str] | None = None,
    ):
        """
        Args:
            headless: 是否无头模式。默认 False（有头模式，支持验证码手动处理）。
            timeout: 登录超时时间（秒），用于手动登录等待。
            required_cookies: ★ F3 完整 Cookie 集——必须**全命中**才判定登录成功。
                不传（None/空集）时保持原行为（仅看 auth cookie 是否出现）。
                来源：`CredentialStore.required_cookies_for()`。
        """
        self.headless = headless
        self.timeout = timeout
        self._cancelled = False
        self.required_cookies: set[str] = {
            str(c).strip() for c in (required_cookies or ()) if str(c).strip()
        }
    ...
    async def login(
        ...
        try:
            ...
            if not result.success:  # 手动登录已成功则跳过
                ...
                # ★ F3：喂入完整 Cookie 集（此前该参数就绪但生产零实参 →
                #   登录态判定退化为"只要出现任意 auth cookie 就算成功"，
                #   会把"只拿到部分 cookie 的半登录态"误判为登录成功）。
                login_ok = await self._wait_for_login_result(
                    page, pre_url, pre_hash, pre_cookie_count,
                    required_cookies=self.required_cookies or None,
                )
    ...
    async def _wait_for_login_result(
        ...
        F3: required_cookies — 必须全命中的 Cookie 名集；
        未传入时保持原有行为（仅检查 auth_cookies 非空）。
        """
        login_fail_keywords = [
        ...
        for _attempt in range(max_wait):
            ...
            try:
                ...
                if url_changed or hash_changed or bool(auth_cookies) or bool(local_token):
                    # F3: 若指定 required_cookies，必须全命中才判定成功
                    if required_cookies:
                        present = {c["name"].lower() for c in post_cookies}
