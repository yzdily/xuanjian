...
from ._blocklist import THIRD_PARTY_DOMAIN_BLOCKLIST
...
# ============================================================
# 异常类：P2 启动原子化 — mitm 链路未就绪时主动 abort，不进入爬取
# ============================================================

class NoTrafficCaptured(RuntimeError):
    """P2 启动原子化守卫抛出：mitm 链路或浏览器代理未就绪，禁止进入 Phase 0。

    触发条件（任一）：
    - mitmdump 进程未在监听
    - Playwright 真连目标站 status >= 500
    - Playwright 探活抛异常
    - 浏览器 launch 显式带 proxy= 后访问目标失败
    """
...
class AutoCrawler(LoginMixin, ScopeMixin, UrlFilterMixin, FormMixin, ResultBuilderMixin, SPAMixin):
    ...
    def __init__(
        ...
        # ★ 已知第三方 SDK / 基础设施域黑名单（绝不爬取）— 单源定义见 _blocklist.py
        self._THIRD_PARTY_BLACKLIST = set(THIRD_PARTY_DOMAIN_BLOCKLIST)
    ...
    async def _crawl_round(self, role: str, login_info: dict | None) -> CrawlRoundResult:
        ...
        # ★ P2 启动原子化（2026-08-28）：use_proxy=True 仍需二次实测，
        # 防止 _check_proxy 的 Playwright 探活通过、但实际 launch 的浏览器
        # 因证书/路径/沙箱等差异走不到代理。所有条件通过才放行。
        if use_proxy:
            from playwright.async_api import async_playwright as _ap_p2
            _p2_pw = None
            _p2_browser = None
            try:
                _p2_pw = await _ap_p2().start()
                _p2_browser = await _p2_pw.chromium.launch(
                    headless=True,
                    proxy={"server": proxy_url},
                    args=["--ignore-certificate-errors"],
                )
                _p2_page = await _p2_browser.new_page()
                _p2_resp = await _p2_page.goto(
                    self.target, timeout=10000, wait_until="domcontentloaded",
                )
                if _p2_resp is None or _p2_resp.status >= 500:
                    raise NoTrafficCaptured(
                        f"P2 启动原子化拦截: 目标 {self.target} 实测 status="
                        f"{_p2_resp.status if _p2_resp else 'None'} ≥ 500，"
                        f"代理链路未真正就绪，禁止进入 Phase 0。"
                    )
                self._report(
                    f"  ✅ P2 代理链路实测通过: {self.target} → {_p2_resp.status}"
                )
            except NoTrafficCaptured:
                # 透传，让外层 abort
                try:
                    if _p2_browser is not None:
                        await _p2_browser.close()
                except Exception:
                    pass
                try:
                    if _p2_pw is not None:
                        await _p2_pw.stop()
                except Exception:
                    pass
                raise
            except Exception as _e:
                # 任何异常都按"链路未就绪"处理，宁严勿松
                try:
                    if _p2_browser is not None:
                        await _p2_browser.close()
                except Exception:
                ...
                try:
                    ...
                    if browser_cookies:
                        ...
                        if relevant:
                            ...
                            # ★ 阶段 4-E1：回写到**会话级作用域**（原写进程级 env → 并行会话互相覆盖）。
                            #   update_scope 保留其它字段，只更新 cookie / target_url，语义与 setdefault 一致。
                            from core.session import cred_scope as _cs
                            _cs.update_scope(
                                cookies=cookie_str,
                                target_url=_cs.current_target_url() or self.target,
                                source=_cs.SOURCE_AUTO_LOGIN,
                            )
                            self._report(f"  🍪 已保存 {len(relevant)} 个登录 Cookie 到会话作用域，后续浏览器实例将自动注入")
                    ...
                    try:
                        ...
                        if local_storage_items:
                            ...
                            # ★ 阶段 4-E1：回写到会话级作用域（原写进程级 env）
                            from core.session import cred_scope as _cs
                            if first_token and not _cs.current_auth():
                                _cs.update_scope(auth=f"Bearer {first_token}")
                            # 供 browser_mcp 浏览器 localStorage 注入：保留任务启动时已推导的自定义 key。
                            existing_items = _cs.current_local_storage()
                            merged_items = {**existing_items, **local_storage_items}
                            _cs.update_scope(local_storage=merged_items)
