...
class LoginMixin:
    ...
    async def _check_proxy(proxy_url: str, target_url: str = "") -> bool:
        """检查代理是否可用 — 2026-08-28 P1 升级：Playwright 真连 + 三重探活。

        旧实现仅用 httpx 探活 mitmproxy 自身，能 200 就算通——
        完全无法验证「浏览器真的会接代理」「目标站能拿到 200」。
        本次升级：
        1) 端口监听（最快失败）
        2) Playwright 真起临时浏览器 + 传 proxy= + 访问目标站
        3) 验响应 < 500（防 502 误判为通过）

        返回 True 仅代表「mitm 进程在 + 浏览器能起 + 目标站 200 链路通」，
        仍不等价于「目标站业务 API 全部可达」，但已足够避免此前 0 流量的根因。
        """
        import socket
        ...
        try:
            ...
            if _sock.connect_ex(("127.0.0.1", port)) != 0:
                _log.warning("mitmproxy 端口 %d 未监听, 切换直连模式", port)
                _sock.close()
                return False
            _sock.close()
        except Exception as e:
            _log.warning("mitmproxy 端口探活异常: %s, 切换直连模式", e)
            return False

        # 2) Playwright 真连（核心：覆盖旧 httpx 探活完全缺失的场景）
        try:
            from playwright.async_api import async_playwright
        except ImportError:
            _log.warning("playwright 未安装, 退化为端口可达即 True（保留旧行为下限）")
            return True

        _test_target = target_url or "http://example.com"
        _pw = None
        _browser = None
        try:
            ...
            try:
                _resp = await _page.goto(
                    _test_target, timeout=10000, wait_until="domcontentloaded",
                )
            except Exception as e:
                _log.warning("mitmproxy 真连失败 (%s): %s, 切换直连模式",
                             _test_target, e)
                return False
            if _resp is None or _resp.status >= 500:
                _log.warning("mitmproxy 真连异常状态 (status=%s), 切换直连模式",
                             _resp.status if _resp else "None")
                return False
            _log.info("mitmproxy 真连成功: %s → status=%d", _test_target, _resp.status)
            return True
        except Exception as e:
            _log.warning("mitmproxy Playwright 探活异常: %s, 切换直连模式", e)
            return False
        finally:
            try:
