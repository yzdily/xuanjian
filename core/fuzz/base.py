...
def _waf_attribution(status: int, headers: dict, body: str) -> dict:
    """§2.7 WAF 厂商归因（失败降级为空归因，绝不影响 WAF 判定）。"""
    try:
        from core.loops.identify_waf import identify

        return identify(status=int(status or 0), headers=headers or {}, body=body or "")
    except Exception:
...
class BaseFuzzer:
    ...
    def __init__(self, proxy_url: str = "", default_timeout: float = 30.0):
        ...
        # §2.7：最近一次 WAF 拦截的厂商归因（无拦截时为空 dict）
        self.last_waf_attribution: dict = {}
    ...
    def _detect_waf(self, status: int, body: str, headers: dict) -> bool:
        """WAF 检测（§2.7 升级：布尔 → 厂商归因）。

        仍返回 bool（调用方契约不变），但命中时会把厂商归因写入
        ``self.last_waf_attribution``，供"按厂商选绕过原语"消费。
        """
        if status in (403, 406, 429, 503):
            ...
            if any(kw in body_lower for kw in waf_keywords):
                self.last_waf_attribution = _waf_attribution(status, headers, body)
                return True
        waf_headers = ["x-waf", "x-sucuri", "x-cdn", "cf-ray", "x-powered-by-anquanbao"]
        if any(h in {k.lower() for k in headers} for h in waf_headers):
            self.last_waf_attribution = _waf_attribution(status, headers, body)
            return True
        return False
