...
# ============================================================
# F16 (0903) — 报告 11 必填字段 + CVSS 代码算分
# strix 核心⑥ HARD：报告 11 必填字段、CVSS 由代码算分
# ============================================================
...
def validate_report(report: dict) -> list[str]:
    ...
    if report.get("severity_vector"):
        try:
            report["severity_score"] = _calc_cvss_score(report["severity_vector"])
        except Exception as e:
            errors.append(f"CVSS 向量非法: {e}")

    return errors


def _calc_cvss_score(vector: str) -> float:
    """从 CVSS 3.x 向量计算基础分。
    ...
    try:
        ...
        calc = CVSS3(vector)
        return float(calc.base_score)
    except ImportError:
        return _calc_cvss_simple(vector)
    except Exception:
        raise ValueError(f"无法解析 CVSS 向量: {vector}")


# CVSS 3.1 基础分简化计算（不依赖外部库时的降级方案）
_CVSS_METRICS = {
...
def _calc_cvss_simple(vector: str) -> float:
    """简化 CVSS 3.1 基础分计算（无需外部库）。"""
    # 去掉 CVSS:3.1/ 或 CVSS:3.0/ 前缀
    clean = vector.strip()
    if clean.startswith("CVSS:"):
        parts = clean.split("/", 1)
        if len(parts) > 1:
            clean = parts[1]
    ...
    if not values.get("AV") or not values.get("AC"):
        raise ValueError("CVSS 向量缺少必填指标 AV/AC")

    scope = values.get("S", "U")
    impact_sub = 1 - (
        (1 - _CVSS_METRICS["C"].get(values.get("C", "N"), 0.0))
        * (1 - _CVSS_METRICS["I"].get(values.get("I", "N"), 0.0))
        * (1 - _CVSS_METRICS["A"].get(values.get("A", "N"), 0.0))
    )
    if scope == "C":
    ...
    exploitability = 8.22 * (
        _CVSS_METRICS["AV"].get(values.get("AV"), 0.85)
    ) * _CVSS_METRICS["AC"].get(values.get("AC"), 0.77) * _CVSS_METRICS["PR"].get(
        values.get("PR", "N"), 0.85
    ) * _CVSS_METRICS["UI"].get(values.get("UI"), 0.85)
