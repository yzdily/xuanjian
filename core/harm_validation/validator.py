...
# ============================================================
# F15 (0903) — 反误报三态闭包
# strix 核心②.5：confirmed / ruled_out / open_proof_gap
# ============================================================
...
# 9 种"看似证伪其实不够"的模式（来自 strix）
INSUFFICIENT_RULED_OUT_PATTERNS = [
    "generic_library_trust",       # 库默认安全
    "control_on_other_path",       # 控制跑在别的路径
    "timing_mismatch",             # 时序错
    "fail_open_present",           # 存在 fail-open
    "safe_sibling_assumption",     # 安全兄弟节点
    "missing_information",         # 信息缺失
    "difficulty_only",             # 只是困难
    "configurability_only",        # 仅可配置
    "internal_only",               # 仅内部
]


def validate_ruled_out(reason: str) -> bool:
    """reason 不能命中任一 INSUFFICIENT 模式。

    返回 True 表示 ruled_out 判定有效；False 表示理由不充分，
    应升级为 open_proof_gap。
    """
    if not reason:
        return False
...
def three_state_verdict(verdict_str: str, reason: str = "") -> Verdict:
    """将二元 verdict + reason 映射为三态。

    accepted → CONFIRMED
    rejected + validate_ruled_out(reason) → RULED_OUT
    rejected + not validate_ruled_out(reason) → OPEN_PROOF_GAP
    borderline / 其他 → OPEN_PROOF_GAP
    """
    if verdict_str == "accepted":
        return Verdict.CONFIRMED
    if verdict_str == "rejected":
        if validate_ruled_out(reason):
            return Verdict.RULED_OUT
        return Verdict.OPEN_PROOF_GAP
    return Verdict.OPEN_PROOF_GAP
