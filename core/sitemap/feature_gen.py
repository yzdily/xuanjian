...
class FeatureGenMixin:
    ...
    def generate_atomic_features(self, crawl_result: dict) -> list[FeaturePoint]:
        ...
        # ---- testflow v2 域归属（灰度 flag，纯分析 0 请求）----
        # flag: XUANJIAN_TESTFLOW_V2 ∈ {census, playbook, full}；未设置 → 存量行为不变
        if os.environ.get("XUANJIAN_TESTFLOW_V2"):
            try:
                from core.testflow.attribution import attribute_domains, attribution_summary
                targets = list(self.features.values()) if getattr(self, "features", None) else created
                stats = attribute_domains(targets, self)
                log.info(attribution_summary(stats))
            except Exception as exc:
                log.warning("testflow attribution 跳过: %s", exc)
